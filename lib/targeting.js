...
Targeting.prototype.wrapUserNames = function(message, userNames, myName) {
  ...
  if (myName) {
    ...
    var myNameRegexp    = new RegExp(this.char+myName, "i");
    message = message.replace(myNameRegexp, "<span class=\"user-targeting target target-self\">$1</span>");
  }

  var userNameRegexp  = new RegExp("("+this.char+"("+userNames.join('|')+"))", "gi");

  message = message.replace(userNameRegexp,       "<span class=\"user-targeting target\">$1</span>");
