...
Targeting.prototype.wrapUserNames = function(message, userNames, myName) {
  userNames.forEach(function(name) {
    message = message.replace(nameRegexp(name), function(match, nameWithAt, name) {
      if (name === myName) {
        return '<span class="user-name target target-self">' + nameWithAt + '</span>';
      } else {
        return '<span class="user-name target">' + nameWithAt + '</span>';
      }
    })
  }, this)
  return message.replace(this.everyoneRegexp, "<span class=\"user-name target target-self\">$1</span>");
};


Targeting.prototype.referencesName = function(message, name) {
  return Boolean(message.match(nameRegexp(name)));
};
